...
import ms from "ms";
import { updatePrem } from "../../wrapper/db/premium.js";
const tiers = [
    "bronze-tier",
    "silver-tier",
    "gold-tier",
...
export default class UpdatePremium extends OrasCommand {
    constructor(client) {
        super(client);
        ...
        this.exec = async (message, args, prefix) => {
            ...
            if (!user)
                return message.reply({
                    embeds: [
                        new this.client.utils.errorEmbed().setDescription(`<:icons_cross:1204719930046423050> Please provide me a valid User`),
                    ],
                });
            if (this.client.utils.checkUserPrem(user.id) === false)
                return message.reply({
                    embeds: [
                        this.client.utils
                            .errorEmbed()
                            .setDescription(`<:icons_cross:1204719930046423050> This User ${user} is not a Premium User`),
                    ],
                });
            let count = 0;
            let tier = "unknown";
                ...
                return message.reply({
                    embeds: [
                        this.client.utils
                            ...
                            .setDescription(`<:Check_Icon:1204720507623178290> Successfully **Updated** Premium to ${user} for ${count} Servers\nwhich will expire on: <t:${time}> (<t:${time}:R>)\nExecutive Code: \`${code}\``),
                    ],
                });
            else
                return message.reply({
